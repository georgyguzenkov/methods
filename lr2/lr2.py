...
def create_gui():
    ...
    def process_file():
        ...
        if file_path:
            try:
                ...
                encrypted_text = playfair_encrypt(text, table)
    ...
    # Кнопка выбора файла
    browse_button = ttk.Button(frame, text="Выбрать файл...", command=process_file)
    browse_button.grid(row=2, column=0, pady=20)

    # Инструкции
    instructions = ttk.Label(frame, text="Выберите текстовый файл для шифрования.\nРезультат будет сохранен в новый файл с суффиксом '_encrypted'.")
